...
public class Game {
    ...
    public void handleEvent(String cardType) {
        Player currentPlayer = getCurrentPlayer();

        if(cardType.startsWith("Plague")){
            currentPlayer.removeShields(2);

        }else if (cardType.startsWith("Queen")){
            //Player draws 2 A cards
            List<Card> drawnCards = adventureDeck.draw(2);
            currentPlayer.addCard(drawnCards.get(0));
            currentPlayer.addCard(drawnCards.get(1));
        ...
        }else if(cardType.startsWith("Prosperity")) {
            for (Player player : players) {
                List<Card> advCards = adventureDeck.draw(2);
                player.addCard(advCards.get(0));
                player.addCard(advCards.get(1));
    ...
    public List<Card> getCurrentStage(){
        if(!gameStages.isEmpty()){
            return gameStages.get(gameStages.size()-1);// .getLast();
        }else{
            return new ArrayList<>();
        }
    }
    public void addGameStage(List<Card> stage){gameStages.add(stage);}
            ...
            if(scanner.hasNextLine()) {
                ...
                if (input.equalsIgnoreCase("quit")) {
                    ...
                    if (choice >= 0 && choice < players[currentPlayer].getHandSize()) {
                        ...
                        if (stage.isEmpty() || c.getValue() > stage.get(stage.size()-1).getValue()) {
                            stage.add(c);
    ...
    public void addUsedEventCard(Card card){
    ...
    public void clearEventDeck() {eventDeck.clear();}

    public void clearAdventureDeck() {adventureDeck.clear();}
    ...
    public String getStageOwner(){return stageOwner.getName();}
    public void setStageOwner(Player player){stageOwner = player;}
    ...
    public void playerAddCards(Player player, String type, int value, int count){
    ...
    public void addCardToEventDeck(Card card) {
